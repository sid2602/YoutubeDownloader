...
const getMovieInfo = (URL) => async (dispatch) => {
  dispatch(Actions.Request());

  try {
    const MovieInfo = await fetchMovie(URL);
    const DataSRC =
      MovieInfo.player_response.microformat.playerMicroformatRenderer;
    let Mp4 = MovieInfo.formats.filter(
      (item) =>
        item.container === "mp4" &&
        item.qualityLabel !== null &&
        item.audioBitrate !== null
    );
    let Webm = MovieInfo.formats.filter(
      (item) => item.container === "webm" && item.qualityLabel !== null
    );

    function Comp(a, b) {
      const qa = parseInt(
        a.qualityLabel.substr(0, a.qualityLabel.indexOf("p"))
      );
      const qb = parseInt(
        b.qualityLabel.substr(0, b.qualityLabel.indexOf("p"))
      );
